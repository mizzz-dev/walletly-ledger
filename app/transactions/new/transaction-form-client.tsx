    ...
    <form action={formAction} className="grid gap-4 lg:grid-cols-[1fr_360px]">
      ...
      <Card className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-1 text-sm">金額<Input name="amount" type="number" min={1} value={amount} onChange={(e) => setAmount(Number(e.target.value))} /></label>
          <label className="space-y-1 text-sm">カテゴリ<Select name="categoryId" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</Select></label>
          <label className="space-y-1 text-sm">支払者<Select name="payerMembershipId" value={payerMembershipId} onChange={(e) => setPayerMembershipId(e.target.value)}>{members.map((member) => <option key={member.membershipId} value={member.membershipId}>{member.name}</option>)}</Select></label>
          ...
          <label className="space-y-1 text-sm">店舗名<Input name="merchant" placeholder="例: JR東日本" value={merchantName} onChange={(e) => setMerchantName(e.target.value)} /></label>
          <label className="space-y-1 text-sm md:col-span-2">メモ<Input name="note" placeholder="例: スーパーまとめ買い" value={note} onChange={(e) => setNote(e.target.value)} /></label>
          <label className="space-y-1 text-sm md:col-span-2">日付<Input name="transactionDate" type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label>
        </div>
        ...
        <Button type="submit" disabled={!canSubmit}>{isPending ? "保存中..." : "支出を保存"}</Button>
      </Card>
      <Card>
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {preview.map((row) => (
            <li key={row.memberId} className="space-y-2 rounded-xl bg-muted/50 px-3 py-2">
              ...
              <Input type="number" min={0} value={row.amount} onChange={(e) => setManualPreview((prev) => ({ ...prev, [row.memberId]: Number(e.target.value) }))} />
            </li>
          ))}
        </ul>
      </Card>
